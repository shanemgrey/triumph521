# Chapter 11 Spec — Resource Rents

## Chapter
- Number: 11
- Working title: Resource Rents
- Target length: ~2,500 words (variance allowed)

## Chapter Job (non-overlapping)
Extend the framework to natural resources and preview why different commons categories need different fee structures (finite vs regenerative; stock vs flow; measurability).
...
- Avoid claiming precise ecological outcomes as guaranteed.

## Key Claims (IDs)
- `C-010`: finite resources can be priced to capture rent and reduce throughput (intuition here).
- `C-015`: fee structure must match category/measurability.

## Evidence Plan
- Thought experiments: what happens when extraction is priced as a commons lease vs as private windfall.

## Glossary / Terms
- Introduce in this chapter: Resource rent (formal definition if not already used).
- Safe to use: LEF, Citizen Dividend, People’s Budget.
